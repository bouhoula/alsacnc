...
from classifiers.cookieblock_classifier.utils import (
    check_flag_changed,
    delim_sep_check,
    load_lookup_from_csv,
    split_delimiter_separated,
    try_decode_base64,
    try_split_json,
...
class CookieFeatureProcessor:
    def __init__(self, feature_parameter_source: str, skip_cmp_cookies: bool) -> None:
        ...
        if skip_cmp_cookies:
            self._skipped_names: Optional[re.Pattern] = re.compile(
        ...
        for f in self.feature_mapping["per_update_features"]:
            ...
            if f["enabled"]:
                f_enabled += 1
                self.num_update_features += (
                    f["vector_size"] * self.feature_mapping["num_updates"]
                )
        ...
        for f in self.feature_mapping["per_diff_features"]:
            ...
            if f["enabled"]:
                f_enabled += 1
                self.num_diff_features += f["vector_size"] * (
                    self.feature_mapping["num_diffs"]
                )
        logger.info(f"Number of per-diff functions: {funcs}")
        logger.info(f"Number of per-diff functions enabled: {f_enabled}")

        self.num_features: int = (
            self.num_cookie_features + self.num_update_features + self.num_diff_features
        )
        ...
        # This set is required to limit false positives.
        # These are all the separators recognized as valid.
        self.valid_csv_delimiters: str = ",|#:;&_.-"

        # Strings that identify boolean values.
        self.truth_values: re.Pattern = re.compile(
            r"\b(true|false|yes|no|0|1|on|off)\b", re.IGNORECASE
        )

        # Horrible Date Regexes
        self.pattern_year_month_day: re.Pattern = re.compile(
            "(19[7-9][0-9]|20[0-3][0-9]|[0-9][0-9])-[01][0-9]-[0-3][0-9]"
        )
        self.pattern_day_month_year: re.Pattern = re.compile(
            "[0-3][0-9]-[01][0-9]-(19[7-9][0-9]|20[0-3][0-9]|[0-9][0-9])"
        )
        self.pattern_month_day_year: re.Pattern = re.compile(
            "[01][0-9]-[0-3][0-9]-(19[7-9][0-9]|20[0-3][0-9])"
        )

        # Day and Month identifiers
        self.pattern_alpha3_days_eng: re.Pattern = re.compile(
            "(Mon|Tue|Wed|Thu|Fri|Sat|Sun)", re.IGNORECASE
        )
        self.pattern_alpha3_months_eng: re.Pattern = re.compile(
            "(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)", re.IGNORECASE
        )
        self.pattern_full_days_eng: re.Pattern = re.compile(
            "(Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday)", re.IGNORECASE
        )
        self.pattern_full_months_eng: re.Pattern = re.compile(
        ...
        self.pattern_canon_uuid: re.Pattern = re.compile(
            "[0-9a-f]{8}-[0-9a-f]{4}-([0-9a-f])[0-9a-f]{3}-[0-9a-f]{4}-[0-9a-f]{12}",
            re.IGNORECASE,
        )
        self.pattern_noncanon_uuid: re.Pattern = re.compile(
            "[0-9a-f]+-[0-9a-f]+-[0-9a-f]+-[0-9a-f]+-[0-9a-f]+", re.IGNORECASE
        )
        self.pattern_http: re.Pattern = re.compile("http(s)?://.*\.")
        self.pattern_www: re.Pattern = re.compile("www(2-9)?\..*\.")
        self.pattern_hexstring: re.Pattern = re.compile(
            "^[0-9a-f]+$", re.IGNORECASE
        )  # want exact match
        ...
        all_features_list: List[Dict[str, Any]] = (
            self.feature_mapping["per_cookie_features"]
    ...
    def _reset_col(self) -> None:
        ...
        assert (
            self.num_features == self._current_col
        ), f"Inconsistent Feature Count {self.num_features} and {self._current_col}"
        self._current_col = 0
    ...
    def _multi_insert_sparse_entries(
        self, data: List[float], col_offset: int = 0
    ) -> None:
        """
        ...
        for feature in self.feature_mapping["per_cookie_features"]:
            if feature["enabled"]:
                for i in range(feature["vector_size"]):
                    feat_list.append(
                        str(feat_cnt + i) + " " + feature["name"] + f"-{i} i"
                    )
    ...
    def retrieve_sparse_matrix(
        ...
        return scipy.sparse.csr_matrix(
            (self._data_entries, (self._row_indices, self._col_indices)), shape=shape
        )

    def retrieve_xgb_matrix(
        self, include_labels: bool, include_weights: bool
    ) -> xgb.DMatrix:
        """
        ...
        """
        assert len(self._data_entries) > 0, "No features stored by processor!"
        assert (not include_labels and not include_weights) or len(
            self._labels
        ) > 0, "No labels stored by processor!"
        ...
        weights: Optional[List[float]] = (
            self.retrieve_label_weights(num_labels=4) if include_weights else None
        )
        return xgb.DMatrix(
            sparse_mat,
            label=labels,
    ...
    def dump_sparse_matrix(
        self, out_path: str, dump_weights: bool = True
    ) -> Tuple[csr_matrix, List[int], List[float]]:
        """
        ...
        dtrain = self.retrieve_sparse_matrix(
            shape=(self._current_row, len(feature_names))
        )
        ...
        weights = None
        if dump_weights and len(Counter(labels).keys()) == 2:
            weights = self.retrieve_label_weights(num_labels=2)
            with open(out_path + ".weights", "wb") as fd:
                pickle.dump(weights, fd)

        return dtrain, labels, weights
    ...
    def retrieve_debug_output(self) -> List[Dict[str, float]]:
        ...
        assert matrix.shape[1] <= len(
            feature_names
        ), f"Number of columns exceeds number of features: Matrix: {matrix.shape[1]} -- Features: {len(feature_names)}"
        ...
        for entry_name, entry_values in input_data.items():
            ...
            for feature in self.feature_mapping["per_cookie_features"]:
                if feature["enabled"]:
                    ...
                    ), f"Defined per-cookie function not found: {feature['function']}"
            ...
            for feature in self.feature_mapping["per_update_features"]:
                if feature["enabled"]:
                    assert hasattr(
                        self, feature["function"]
                    ), f"Per-update function not found: {feature['function']}"
                    ...
                    try:
                        ...
                        empty_update_slots = feature["vector_size"] * (
                            self.feature_mapping["num_updates"] - update_count
                        )
            ...
            for feature in self.feature_mapping["per_diff_features"]:
                if feature["enabled"]:
                    assert hasattr(
                        self, feature["function"]
                    ), f"Difference function not found: {feature['function']}"
                    v_iter = iter(entry_values["variable_data"])
                    function = getattr(self, feature["function"])
                    ...
                    try:
                        ...
                        # if out of updates, need to increment column counter so size is uniform
                        empty_update_slots = feature["vector_size"] * (
                            self.feature_mapping["num_diffs"] - update_count
                        )
                        self._increment_col(empty_update_slots)
    ...
    def extract_features_with_labels(
        self, input_data: Dict[str, Dict[str, Any]], disable_tqdm: bool = False
    ) -> None:
        """
        ...
        :param disable_tqdm: Indicates whether to show tqdm progress bar
        """
        ...
        for entry_name, entry_values in tqdm(input_data.items(), disable=disable_tqdm):
            # retrieve the label and skip ones we don't want
            # TODO add parameter to indicate whether we return a binary or quaternary label
            category_label = int(entry_values["label"])
            if category_label in [0, 1]:
                category_label = 0
            elif category_label in [2, 3]:
                category_label = 1

            # Make sure we only consider desired labels
            if not (0 <= category_label <= 1):
                ctr_label_skipped += 1
                continue
            ...
            for feature in self.feature_mapping["per_cookie_features"]:
                if feature["enabled"]:
                    ...
                    timings_per_function[feature["function"]].append(
        ...
        logger.info(
            f"Feature extraction completed. Final row position: {self._current_row}"
        )

        logger.info("Timings per feature:")
        total_time_spent = 0
        for func, t_list in sorted(
            timings_per_function.items(), key=lambda x: sum(x[1]), reverse=True
        ):
            if len(t_list) == 0:
                continue
            else:
                ...
                logger.info(logmsg)
        logger.info(
            f"Total time spent in feature extraction: {total_time_spent / 1e9:.3f} seconds"
        )
        logger.info(
            f"Time lost to overhead: {total_time_taken - (total_time_spent / 1e9):.3f} seconds"
        )
    ...
    def setup_pattern_names(self, source: str, vector_size: int) -> None:
        ...
        with open(source) as fd:
            ...
            try:
                ...
                raise RuntimeError(
                    f"Not enough patterns in file. Expected at least {vector_size}, max is {rank}."
                )
    ...
    def feature_top_names(self, cookie_features: Dict[str, Any]) -> None:
        ...
        """
        assert (
            self._top_name_lookup is not None
        ), "Top N name lookup was not set up prior to feature extraction!"
    ...
    def feature_top_domains(self, cookie_features: Dict[str, Any]) -> None:
        ...
        """
        assert (
            self._top_domain_lookup is not None
        ), "Top N domain lookup was not set up prior to feature extraction!"
        cookie_domain: str = url_to_uniform_domain(cookie_features["cookie_domain"])
    ...
    def feature_pattern_names(self, cookie_features: Dict[str, Any]) -> None:
        ...
        """
        assert (
            self._pattern_names is not None
        ), "Top N pattern lookup was not set up prior to feature extraction!"
        cookie_name: str = cookie_features["name"]
    ...
    def feature_name_tokens(self, cookie_features: Dict[str, Any]) -> None:
        ...
        """
        assert (
            self._name_features is not None
        ), "Top N name features map was not set up prior to feature extraction!"
    ...
    def feature_iab_vendor(self, cookie_features: Dict[str, Any]) -> None:
        ...
        """
        assert (
            self._iab_europe_vendors is not None
        ), "IAB Europe Vendors Set not initialized!"
        sanit_domain = url_to_uniform_domain(cookie_features["cookie_domain"])
    ...
    def feature_is_third_party(self, cookie_features: Dict[str, Any]) -> None:
        ...
        """
        cookie_domain = url_to_uniform_domain(cookie_features["cookie_domain"])
        website_domain = url_to_uniform_domain(cookie_features["website"])
            ...
            while previous_entry is not None:
                ...
                if curr_entry is not None:
                    matcher = difflib.SequenceMatcher(
                        a=previous_entry["value"], b=curr_entry["value"]
                    )
    ...
    def feature_levenshtein_mean_and_stddev(
        self, cookie_features: Dict[str, Any]
    ) -> None:
        """per-cookie update
        ...
        try:
            while previous_entry is not None:
                curr_entry = next(v_iter)
                if curr_entry is not None:
                    lev_metric = lev_distance(
                        previous_entry["value"], curr_entry["value"]
                    )
    ...
    def feature_content_length_mean_and_stddev(
        self, cookie_features: Dict[str, Any]
    ) -> None:
        """per-cookie update
    ...
    def feature_compressed_length_mean_and_stddev(
        self, cookie_features: Dict[str, Any]
    ) -> None:
        """per-cookie update
    ...
    def feature_entropy_mean_and_stddev(self, cookie_features: Dict[str, Any]) -> None:
        ...
        for cookie_update in cookie_features["variable_data"]:
            ...
            entropy: float = 0
            for ratio in [
                char_count / total_string_size
                for char_count in content_char_counts.values()
            ]:
    ...
    def feature_expiry_extra(self, var_data: Dict[str, Any]):
        ...
        """
        # less than 1 hour
        self._insert_sparse_entry(
            1.0 if var_data["expiry"] < 3600 else -1.0, col_offset=0
        )
        # between 1 and 12 hours
        self._insert_sparse_entry(
            1.0 if 3600 <= var_data["expiry"] <= 3600 * 12 else -1.0, col_offset=1
        )
        # between 12 to 24 hours
        self._insert_sparse_entry(
            1.0 if 3600 * 12 <= var_data["expiry"] <= 3600 * 24 else -1.0, col_offset=2
        )
        # between 1 to 7 days
        self._insert_sparse_entry(
            1.0 if 3600 * 24 <= var_data["expiry"] <= 3600 * 24 * 7 else -1.0,
            col_offset=3,
        )
        # between 1 week to 1 month
        self._insert_sparse_entry(
            1.0 if 3600 * 24 * 7 <= var_data["expiry"] <= 3600 * 24 * 30 else -1.0,
            col_offset=4,
        )
        # between 1 month to 6 months
        self._insert_sparse_entry(
            1.0 if 3600 * 24 * 30 <= var_data["expiry"] <= 3600 * 24 * 30 * 6 else -1.0,
            col_offset=5,
        )
        # between 6 months to 18 months
        self._insert_sparse_entry(
            1.0
            if 3600 * 24 * 30 * 6 <= var_data["expiry"] <= 3600 * 24 * 30 * 18
            else -1.0,
            col_offset=6,
        )
        # more than 18 months
        self._insert_sparse_entry(
            1.0 if 3600 * 24 * 30 * 18 <= var_data["expiry"] else -1.0, col_offset=7
        )
    ...
    def feature_delimiter_separated(
        self, var_data: Dict[str, Any], min_seps: int
    ) -> None:
        """per-update feature:
        ...
        """
        unquoted_content = urllib.parse.unquote(var_data["value"])
        split_data, delimiter = split_delimiter_separated(
            unquoted_content,
            self.csv_sniffer,
            delimiters=self.valid_csv_delimiters,
            min_seps=min_seps,
        )
    ...
    def feature_english_terms_in_content(self, var_data: Dict[str, Any]) -> None:
        ...
        """
        assert (
            self._content_terms is not None
        ), "Content terms were not set up prior to feature extraction!"
        unquoted_content: str = urllib.parse.unquote(var_data["value"])
    ...
    def feature_csv_content(self, var_data: Dict[str, Any], min_seps: int) -> None:
        ...
        """
        unquoted_content = urllib.parse.unquote(var_data["value"])
        data_entries, separator = split_delimiter_separated(
            unquoted_content,
            self.csv_sniffer,
            delimiters=self.valid_csv_delimiters,
            min_seps=min_seps,
            ...
            if type(data_dict) is dict:
                ...
                for v in data_dict.values():
                    if type(v) in (int, float):
                        ...
                        logger.error(
                            "Unexpected type of value inside dict:" + str(type(v))
                        )
    ...
    def feature_hex_content(self, var_data: Dict[str, Any]) -> None:
        ...
        """
        unquoted_content = urllib.parse.unquote(var_data["value"])
        if not unquoted_content.isnumeric() and self.pattern_hexstring.match(
            unquoted_content
        ):
            self._insert_sparse_entry(1.0)
        else:
    ...
    def feature_is_identifier(self, var_data: Dict[str, Any]) -> None:
        ...
        unquoted_content = urllib.parse.unquote(var_data["value"])
        if (
            not (unquoted_content.isalpha() or unquoted_content.isnumeric())
    ...
    def feature_date_content(self, var_data: Dict[str, Any]) -> None:
        ...
        if (
            self.pattern_year_month_day.search(unquoted_content)
            or self.pattern_day_month_year.search(unquoted_content)
            or self.pattern_month_day_year.search(unquoted_content)
        ):
            self._insert_sparse_entry(1.0)
        # Check for month + day names
        elif (
            self.pattern_alpha3_days_eng.search(unquoted_content)
            or self.pattern_full_days_eng.search(unquoted_content)
        ) and (
            self.pattern_alpha3_months_eng.search(unquoted_content)
            or self.pattern_full_months_eng.search(unquoted_content)
        ):
            self._insert_sparse_entry(1.0)
    ...
    def feature_url_content(self, var_data: Dict[str, Any]) -> None:
        ...
        """
        unquoted_content = urllib.parse.unquote(var_data["value"])
        if self.pattern_http.search(unquoted_content) or self.pattern_www.search(
            unquoted_content
        ):
            self._insert_sparse_entry(1.0)
        else:
    ...
    def feature_time_diff(
        self, prev_update: Dict[str, Any], curr_update: Dict[str, Any]
    ) -> None:
        """per-diff feature:
    ...
    def feature_gestalt_pattern_ratio(
        self, prev_update: Dict[str, Any], curr_update: Dict[str, Any]
    ) -> None:
        """per-diff feature:
    ...
    def feature_delimiter_separated_new(
        self, var_data: Dict[str, Any], min_seps: int
    ) -> None:
        """per-update feature
        ...
        """
        unquoted_content = urllib.parse.unquote(var_data["value"])
        chosen_delimiter, count = delim_sep_check(
            unquoted_content, self.valid_csv_delimiters, min_seps
        )
    ...
    def feature_period_separated_new(
        self, var_data: Dict[str, Any], min_seps: int
    ) -> None:
        """per-update feature:
    ...
    def feature_dash_separated_new(
        self, var_data: Dict[str, Any], min_seps: int
    ) -> None:
        """per-update feature:
    ...
    def feature_domain_period(self, cookie_features: Dict[str, Any]) -> None:
        ...
        :param cookie_features: Dictionary containing key "domain" of the cookie
        """
        if cookie_features["cookie_domain"].startswith("."):
            self._insert_sparse_entry(1.0)
